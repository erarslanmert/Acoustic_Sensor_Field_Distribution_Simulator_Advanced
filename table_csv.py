...
class Ui_Dialog(object):
    def setupUi(self, Dialog):
        ...
        v_header=[' ']+mainWindow.imported_sensor_names[0]


        self.table.setColumnCount(len(Cp.headers)+len(mainWindow.imported_sensor_names[0])+len(mainWindow.imported_sensor_names[0]))
        self.table.setHorizontalHeaderLabels(Cp.headers+mainWindow.imported_sensor_names[0]+mainWindow.imported_sensor_names[0])
        self.table.setStyleSheet("QHeaderView::section {background-color: #F8F4E2;}")

        self.table1.setColumnCount(len(h_header))
        self.table1.setRowCount(len(v_header)*len(mainWindow.imported_events))
        ...
        self.table1.setVerticalHeaderLabels(v_header*len(mainWindow.imported_events))

        self.table2.setColumnCount(len(mainWindow.imported_events)*2)
        ...
        for x in range(0,len(mainWindow.imported_sensor_names[0])):
            for c in range(0,len(mainWindow.imported_events)):
                single_seconds.append(Cp.table_data_launch[c][(5+len(mainWindow.imported_sensor_names[0])+x)])
                single_seconds.append(Cp.table_data_impact[c][(5 + len(mainWindow.imported_sensor_names[0]) + x)])
            sorter = sorted(single_seconds)
            self.sensor_seconds.append(sorter)
            single_seconds = []
            sorter = []

        for y in range(0,len(mainWindow.imported_sensor_names[0])):
            self.addTableRow_2(self.sensor_seconds[y])
        self.table2.setVerticalHeaderLabels(mainWindow.imported_sensor_names[0])

        self.sensor_info = []
        sensor_individual = []
        for m in range(0, len(mainWindow.imported_events)):
            for n in range(0, len(mainWindow.imported_sensor_names[0])):
                sensor_individual.append(Cp.actual_sensor_coord[0][n])
                ...
                sensor_individual.append(Cp.bearing_sensor_launch[(m * len(mainWindow.imported_sensor_names[0])) + n])
                sensor_individual.append(Cp.bearing_sensor_impact[(m * len(mainWindow.imported_sensor_names[0])) + n])
                ...
                sensor_individual.append(Cp.dist_sensor_launch[(m * len(mainWindow.imported_sensor_names[0])) + n])
                sensor_individual.append(Cp.dist_sensor_impact[(m * len(mainWindow.imported_sensor_names[0])) + n])
                sensor_individual.append(Cp.dist_launch_impact[m])
                if mainWindow.imported_events[m][6] == 1:
                    sensor_individual.append((Cp.dist_launch_impact[m]/float(mainWindow.imported_events[m][7])))
                else:
        ...
        for j in range(0, len(mainWindow.imported_events)):
            self.table1.setSpan(j*(len(mainWindow.imported_sensor_names[0])+1), 0, 1, 16)
            cell = QTableWidgetItem('Event ' + str(j + 1))
            cell.setTextAlignment(Qt.AlignCenter)
            cell.setBackground(QtGui.QColor(255, 180, 110))
            self.table1.setItem(j * (len(mainWindow.imported_sensor_names[0]) + 1), 0, cell)

        for k in range(0,len(mainWindow.imported_events)):
            for a in range(0,len(mainWindow.imported_sensor_names[0])):
                col = 0
                for item in self.sensor_info[k*len(mainWindow.imported_sensor_names[0]) + a]:
                    cell_2 = QTableWidgetItem(str(item))
                    cell_2.setTextAlignment(Qt.AlignCenter)
                    self.table1.setItem(k * (len(mainWindow.imported_sensor_names[0])+1) + a + 1, col, cell_2)
                    col = col + 1

        for i in range(0,len(mainWindow.imported_events)):
    ...
    def addTableRow(self,row_data):
        ...
        for item in row_data:
            ...
            cell.setTextAlignment(Qt.AlignCenter)
            if col > (4+len(mainWindow.imported_sensor_names[0])):
    ...
    def exportExcel(self):
        ...
        if self.const == 1:
            ...
            sheet = wb.active
            for j in range(0, len(mainWindow.imported_events)):
                sheet.cell(row = (j * (len(mainWindow.imported_sensor_names[0]) + 1) + 2), column = 5).value = 'Event ' + str(j + 1)
            for k in range(0, len(mainWindow.imported_events)):
                for a in range(0, len(mainWindow.imported_sensor_names[0])):
                    col = 2
                    sheet.cell(row=(k * (len(mainWindow.imported_sensor_names[0]) + 1) + a + 3), column=1).value = mainWindow.imported_sensor_names[0][a]
                    for item in self.sensor_info[k * len(mainWindow.imported_sensor_names[0]) + a]:
                        sheet.cell(row=(k * (len(mainWindow.imported_sensor_names[0]) + 1) + a + 3), column=col).value = str(item)
                        col = col + 1
            wb.save(response[0])
            ...
            for i in range(0, len(mainWindow.imported_sensor_names[0])):
                sheet.cell(row=i+1 , column=1).value = mainWindow.imported_sensor_names[0][i]
    ...
    def writeCsv(self):
        ...
        elements = []
        coordinates = []
        for act in mainWindow.imported_events:
            elements.append(act[0])
            elements.append(act[2])
            coordinates.append(act[1])
            coordinates.append(act[3])
        colors = ['#FC766AFF','#C1FE20','#20FE7B','#20FEB1','#FCC160','#FE9D35','#FF0909','#FFD100F','#FCFF30','#17F8FF','#5617FF','#F117FF']
        ...
        for event in range(0,len(mainWindow.imported_events)):
            op = 0.32
            for j in range(0,15):
                df.at[round(Cp.launch_times[event] + j*2) * len(elements) + elements.index(mainWindow.imported_events[event][0]), 'Radius'] = (float(mainWindow.imported_events[event][-1]) / 10.75) * (j*2)
                df.at[round(Cp.impact_times[event] + j*2) * len(elements) + elements.index(mainWindow.imported_events[event][2]), 'Radius'] = (float(mainWindow.imported_events[event][-1]) / 10.75) * (j*2)
                df.at[round(Cp.launch_times[event] + j*2 ) * len(elements) + elements.index(mainWindow.imported_events[event][0]), 'Color'] = colors[event]
                df.at[round(Cp.impact_times[event] + j*2 ) * len(elements) + elements.index(mainWindow.imported_events[event][2]), 'Color'] = colors[event]
                df.at[round(Cp.launch_times[event] + j*2 ) * len(elements) + elements.index(mainWindow.imported_events[event][0]), 'Opacity'] = op
                df.at[round(Cp.impact_times[event] + j*2 ) * len(elements) + elements.index(mainWindow.imported_events[event][2]), 'Opacity'] = op
                op = op - 0.02
